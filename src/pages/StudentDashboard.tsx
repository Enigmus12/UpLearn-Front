...
function resolveTimestamp(m: ChatMessageData | any): string {
  return m?.createdAt ?? m?.timestamp ?? new Date().toISOString();
}
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setMessages(hist.map(h => mapAnyToServerShape(h as any, cid!)));
      } catch (e) {

  ...
  useEffect(() => {
    ...
    const fetchStudentData = async () => {
      ...
      try {
        ...
        const reservationsByStudent: Record<string, Reservation[]> = {};
        for (const res of reservations) {
          if (!reservationsByStudent[res.studentId]) {
            reservationsByStudent[res.studentId] = [];
          }
          reservationsByStudent[res.studentId].push(res);
        }
        ...
        const profilePromises = studentIds.map(id => 
          ...
          }).then(res => res.ok ? res.json() : Promise.reject(`Failed to fetch profile for ${id}`))
        );
        ...
        const studentData = studentIds.map(id => {
          const studentReservations = reservationsByStudent[id].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
          const lastSessionDate = studentReservations.find(r => r.status === 'FINALIZADA')?.date;
          const isActive = lastSessionDate && (new Date().getTime() - new Date(lastSessionDate).getTime()) < 30 * 24 * 60 * 60 * 1000;
        ...
        setStudents(studentData.sort((a, b) => (b.status === 'active' ? 1 : -1) - (a.status === 'active' ? -1 : 1) || a.profile.name!.localeCompare(b.profile.name!)));
